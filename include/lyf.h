...
#define getWholeTypeName(x) \
    lyf::type_class<decltype(x)>::get() // 用于获取变量类型的快捷调用宏, 此方法会获取完整的类型, 包括默认参数类型
...
inline void
set_printDelim(std::string_view delim = " ") {
    printDelim = delim;
}
...
inline void
set_printDelimPersist(bool flag = false) {
    delimIsPersist = flag;
}
...
///
/// 2. 对于不继承直接使用单例模式的方式Singleton<Foo2>::GetInstance().printAddress(); 这种方式没有阻止单例的拷贝或赋值,
/// 不推荐使用, 会导致单例失效 例如: auto f2 = Singleton<Foo2>::GetInstance();
